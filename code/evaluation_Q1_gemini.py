...
def main():
    ...
    if 'Fine_novelty_gemini' not in df.columns:
        df['Fine_novelty_gemini'] = ''  # Initialize the 'Novelty' column for all rows
    ...
    for index, row in df.iterrows():
        ...
        if os.path.isfile(file_path):
            ...
            #     df.loc[index, 'Novelty_gemini'] = 'N/A'
            ...
            if row['Novelty_major_voting'] == 'YES' and k < len(solutions):
                ...
                prompts.append(np.nan)
                df.loc[index, 'Fine_novelty_gemini'] = 'N/A'
    ...
    # prompts = prompts.dropna().reset_index(drop=True)
    ...
    for i, prompt in prompts.items():
        ...
        #     continue
        if pd.notna(df.at[i, 'Fine_novelty_gemini']) and df.at[i, 'Fine_novelty_gemini'] != '':
            continue
        ...
        while retry_count < max_retries:
            try:
                ...
                df.at[i, 'Fine_novelty_gemini'] = response
                df.to_csv(output_csv_file, index=False)
                time.sleep(1)
                break
