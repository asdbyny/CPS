...
def main():
    ...
    if 'Fine_novelty_gpt4' not in df.columns:
        df['Fine_novelty_gpt4'] = ''  # Initialize the 'Novelty' column for all rows
    ...
    for index, row in df.iterrows():
        ...
        if os.path.isfile(file_path):
            ...
            if row['Novelty_major_voting'] == 'YES' and k < len(solutions):
                ...
                prompts.append(np.nan)
                df.loc[index, 'Fine_novelty_gpt4'] = 'N/A'
    ...
    for i, prompt in prompts.items():
        ...
        #     continue
        if pd.notna(df.at[i, 'Fine_novelty_gpt4']) and df.at[i, 'Fine_novelty_gpt4'] != '':
            continue
        # prompt style
        ...
        df.at[i, 'Fine_novelty_gpt4'] = response
